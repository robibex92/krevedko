...
import { buildTelegramMessage, sendTelegramMessage } from "../services/telegram.js";
...
router.patch("/admin/collections/:id/activate", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const id = Number(req.params.id);
    const now = new Date();
    const result = await prisma.collection.update({ where: { id }, data: { status: "ACTIVE", startsAt: now, endsAt: null } });
    clearCache("collections_active");
...
router.patch("/admin/collections/:id/close", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const id = Number(req.params.id);
    const now = new Date();
    const col = await prisma.collection.update({ where: { id }, data: { status: "CLOSED", endsAt: now } });
    clearCache("collections_active");
...
router.patch("/admin/products/:id/stock", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    ...
    const data = {};
    if (stockQuantity !== undefined) data.stockQuantity = String(stockQuantity);
...
router.get("/admin/products/low-stock", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const raw = await prisma.product.findMany({ where: { isActive: true }, select: { id: true, title: true, imagePath: true, unitLabel: true, stockQuantity: true, minStock: true } });
    const { dec } = await import("../utils/decimal.js");
    const products = raw.filter((p) => dec(p.stockQuantity).lte(dec(p.minStock))).sort((a, b) => dec(a.stockQuantity).cmp(dec(b.stockQuantity)));
    res.json({ products });
  } catch (err) {
    console.error(err);
...
router.patch("/admin/collections/:collectionId/products/:productId", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const cp = await prisma.collectionProduct.upsert({
      ...
      update: {
        priceOverrideKopecks: priceOverrideKopecks !== undefined ? Number(priceOverrideKopecks) : undefined,
        stepOverrideDecimal: stepOverrideDecimal !== undefined ? String(stepOverrideDecimal) : undefined,
      ...
      create: {
        collectionId,
        productId,
        priceOverrideKopecks: priceOverrideKopecks !== undefined ? Number(priceOverrideKopecks) : null,
        stepOverrideDecimal: stepOverrideDecimal !== undefined ? String(stepOverrideDecimal) : null,
        isActive: isActive !== undefined ? Boolean(isActive) : true,
...
router.get("/admin/orders", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const orders = await prisma.order.findMany({
      include: {
        user: { select: { id: true, name: true, email: true, phone: true, telegramUsername: true, firstName: true, lastName: true, avatarPath: true } },
    ...
    const normalizedOrders = orders.map((order) => ({
      ...
      items: order.items.map((item) => ({ ...item, quantityDecimal: item.quantityDecimal.toString() })),
    }));
...
router.patch("/admin/orders/:id/status", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!["SUBMITTED", "PAID", "CANCELLED"].includes(status)) return res.status(400).json({ error: "INVALID_STATUS" });
...
router.patch("/admin/orders/:id/delivery", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const id = Number(req.params.id);
    ...
    data.deliveryAddress = deliveryType === "DELIVERY" ? (deliveryAddress || null) : null;
...
router.post("/admin/orders/:id/payment-proof", requireAuth, requireAdmin, paymentUpload.single("image"), async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const id = Number(req.params.id);
    const { note } = req.body || {};
    ...
    const proof = await prisma.paymentProof.create({ data: { orderId: order.id, imagePath: relPath, note: note || null } });
...
router.post("/admin/broadcast", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const roles = Array.isArray(filters.roles) ? filters.roles.filter((r) => typeof r === "string" && r.trim()) : [];
    const statuses = Array.isArray(filters.statuses) ? filters.statuses.filter((s) => typeof s === "string" && s.trim()) : [];
    const collectionIds = Array.isArray(filters.collectionIds) ? filters.collectionIds.map((id) => Number(id)).filter((id) => Number.isInteger(id)) : [];
    const userIds = Array.isArray(filters.userIds) ? filters.userIds.map((id) => Number(id)).filter((id) => Number.isInteger(id)) : [];
    ...
    if (statuses.length || collectionIds.length) {
      userWhere.orders = { some: { ...(statuses.length ? { status: { in: statuses } } : {}), ...(collectionIds.length ? { collectionId: { in: collectionIds } } : {}) } };
    }
    ...
    if (wantTelegram) {
      ...
      recipients = await prisma.user.findMany({ where: userWhere, select: { id: true, telegramId: true, name: true, email: true } });
    }
    if (!recipients.length) return res.json({ preview: buildTelegramMessage(message), totalRecipients: 0, sent: 0, failures: [], warnings: channels.includes("EMAIL") ? ["EMAIL channel not configured; skipped"] : [] });
    ...
    res.json({ preview: finalMessage, totalRecipients: recipients.length, sent, failures, warnings: channels.includes("EMAIL") ? ["EMAIL channel not configured; skipped"] : [] });
  } catch (err) {
    console.error(err);
...
router.get("/admin/collections", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const list = await prisma.collection.findMany({ orderBy: [{ status: "desc" }, { id: "desc" }] });
...
router.patch("/admin/collections/:id", requireAuth, requireAdmin, async (req, res) => {
  const prisma = req.app.locals.prisma;
  try {
    const id = Number(req.params.id);
    const { title, startsAt, endsAt, notes } = req.body || {};
