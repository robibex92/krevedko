...
router.post("/admin/broadcast", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    if (!TELEGRAM_BOT_TOKEN || !TELEGRAM_BOT_USERNAME) return res.status(501).json({ error: "TELEGRAM_NOT_CONFIGURED" });
    ...
    const recipients = await prisma.user.findMany({ where: userWhere, select: { id: true, telegramId: true, name: true, email: true } });
    if (!recipients.length) return res.json({ preview: buildTelegramMessage(message), totalRecipients: 0, sent: 0, failures: [] });
    ...
    res.json({ preview: finalMessage, totalRecipients: recipients.length, sent, failures });
  } catch (err) {
    console.error(err);
...
router.patch("/admin/collections/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const id = Number(req.params.id);
    const { title, endsAt, notes } = req.body || {};
    const data = {};
    if (title !== undefined) data.title = title;
