  ...
  try {
    ...
    const [items, total] = await prisma.$transaction([
      prisma.publicReview.findMany({
        orderBy: sort === "rating" ? { rating: direction } : { createdAt: direction },
        skip: (page - 1) * pageSize,
        take: pageSize,
        include: { user: { select: { id: true, name: true, firstName: true, lastName: true, avatarPath: true } }, images: true },
      }),
    ...
    res.json({ reviews, page, pageSize, total, totalPages: Math.ceil(total / pageSize) });
  } catch (err) {
    console.error(err);
